...
import Henceforth from '@/../public/henceforth.png'

interface InquiryDialogProp {
    data: PropertyType,
...
export default function InquiryDialog({data, projectId}:InquiryDialogProp) {
    const project = useQuery(api.projects.getById, {id: projectId ?? undefined})
    const [dialogOpen, setDialogOpen] = useState<boolean>(false)
    const buyer = useQuery(api.users.userById)
        ...
        <TooltipProvider>
            <Tooltip>
                ...
                <DialogTrigger onClick={()=> setDialogOpen(!dialogOpen)}>
                   <Mail className='size-4 cursor-pointer'/>
                </DialogTrigger>
                </TooltipTrigger>
                <TooltipContent>
                    <p>Contact Agent</p>
            ...
            <div className="grid grid-cols-12 gap-2 items-center text-[#A1A7AE] text-xs md:text-sm">
                <div className="col-span-12 md:col-span-5 flex items-center gap-x-2 ">
                    ...
                    <Avatar className='size-8 md:size-12'>
                        <AvatarImage src={buyer?.image} alt="@shadcn" className='size-5' />
                        <AvatarFallback>CN</AvatarFallback>
                    </Avatar>
                    <p className='capitalized text-black font-sans font-semibold tracking-wide'>{buyer?.fname} {buyer?.lname}</p>
                </div>
                <div className="col-span-12 md:col-span-4 flex items-center gap-x-2">
                    ...
                    <span className='capitalized text-black font-sans font-semibold tracking-wide'>{buyer?.email}</span>
                </div>
                <div className="col-span-12 md:col-span-3 flex items-center gap-x-2">
                    ...
                    <span className='capitalized text-black font-sans font-semibold tracking-wide'>{buyer?.contact}</span>
                </div>
            </div>
            <div className="text-[#A1A7AE] text-xs md:text-sm">
                <div className="flex items-center gap-x-2">
                    <h3 className='col-span-6 w-10 md:w-11 text-right'>To :</h3>
                    <Image src={Henceforth} alt='' height={100} width={100} className='rounded-full object-contain size-8 md:size-12'/>
                        
                    <p className='capitalized text-black font-sans font-semibold tracking-wide'>Customer support</p>
                </div>
            </div>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input defaultValue={"Property Inquiry"} placeholder="Enter Subject" {...field} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
